...
如果你能看到这个文档，说明 [脚本](sphinx_doc/build_versions.py) 成功在 Sphinx 构建前，将项目中的外部 Markdown 文档复制到了 Sphinx 源目录（`docs/sphinx_doc/source/`，保留原有相对路径），从而使其能被正常构建。
...
只要这些文件能被 `docs/sphinx_doc/source/index.rst`（英文）和 `index_ZH.rst`（中文）中的 `:glob:` toctree 匹配到，通常它们会像本文档一样出现在最终站点中。
...
`docs/*` 将匹配所有从 `docs/` 目录复制过来的文件，例如本文档。每个带 caption 的 toctree 会在左侧边栏渲染为默认展开的分组。
